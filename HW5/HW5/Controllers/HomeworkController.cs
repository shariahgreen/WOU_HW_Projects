...
namespace HW5.Controllers
{
    ...
    {
        private HomeworkContext dbo = new HomeworkContext();

        // GET: Homework
        public ActionResult Index()
        {
            //SORT HERE
            var homeworksList = from h in dbo.MyHomework select h;
            homeworksList = homeworksList.OrderByDescending(h => h.DueDate);
            return View(homeworksList.ToList());
            //return View(dbo.Homework.ToList());
        }
        ...
        public ActionResult Details(int? id)
        {
            ...
            Homework homework = dbo.MyHomework.Find(id);
            if (homework == null)
            {
                return HttpNotFound();
            }
        ...
        public ActionResult Edit([Bind(Include = "ID,Title,DueDate,DueTime,HWPriority,Department,Course,Notes")] Homework homework)
        {
            if (ModelState.IsValid)
            {
                dbo.Entry(homework).State = EntityState.Modified;
                dbo.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(homework);
        ...
        {
            Homework homework = dbo.MyHomework.Find(id);
            dbo.MyHomework.Remove(homework);
            dbo.SaveChanges();
            return RedirectToAction("Index");
        }
